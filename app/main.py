...
def plot_distributions(df):
    ...
    # Définir le nombre de colonnes par ligne à 2
    ncols =7
    
    # Calculer le nombre de lignes nécessaires
    nrows = (len(numeric_columns) + ncols - 1) // ncols
    
    # Créer une figure avec `nrows` lignes et `ncols` colonnes
    ...
    if request.method == 'POST':
        ...
        if file and file.filename.endswith('.arff'):
            ...
            if 'split' in request.form:
                ...
                # Rendre le template `split.html` avec les données
                return render_template('index2.html', train_rows=train_rows, test_rows=test_rows, train_html=train_html, test_html=test_html)
